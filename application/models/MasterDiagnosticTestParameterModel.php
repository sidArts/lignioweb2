...
class MasterDiagnosticTestParameterModel extends MY_Model {
	...
	public function getTestParameterWithValueByBookingDetailId($diagnostic_test_id, $booking_detail_id) {
		$sql = "select max(lab_analysis_detail_id) as lab_analysis_detail_id from diagnostic_test_results where booking_detail_id = " . $booking_detail_id;
		$query = $this->db->query($sql);
		$result = $query->row_array();
		if($result['lab_analysis_detail_id'] != NULL && $result['lab_analysis_detail_id'] != 0):
			$lab_analysis_detail_id = $result['lab_analysis_detail_id'];
